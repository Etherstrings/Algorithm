...
public class BuildBinaryTree {
    ...
    ACMBulidTree.TreeNode build(List<Integer> vec){
        List<ACMBulidTree.TreeNode> vecTree=new ArrayList<>(vec.size());
        Collections.fill(vecTree,null);
        ACMBulidTree.TreeNode root=null;
        for(int i=0;i<vec.size();i++){
            ACMBulidTree.TreeNode node=null;
            if(vec.get(i)!=-1){
