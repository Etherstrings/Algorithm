 ...
 */
public class BM2 {
    public LinkedList.ListNode reverseBetween (LinkedList.ListNode head, int m, int n) {
        // write code here
    ...
    static LinkedList.ListNode arrayToListNode(int[] s){
        LinkedList.ListNode root=new LinkedList.ListNode(s[0]);
        LinkedList.ListNode other=root;
        for(int i=1;i<s.length;i++){
            LinkedList.ListNode temp=new ListNode(s[i]);
            other.next=temp;
            other=temp;
        }
        return root;
