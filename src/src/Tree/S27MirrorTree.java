...
public class S27MirrorTree {
    //迭代
    //反转重构二叉树
    public Tree.TreeNode mirrorTree(Tree.TreeNode root) {
        if(root==null){
            return root;
        }
        ...
        while(!judge.isEmpty()){
            ...
            for(int i=0;i<size;i++){
                Tree.TreeNode now=judge.poll();
                Tree.TreeNode temp=now.left;
                now.left=now.right;
                now.right=temp;
                if(now.left!=null){
    ...
    //递归
    public Tree.TreeNode mirrorTree1(Tree.TreeNode root){
            return swap(root);

    }

    Tree.TreeNode swap(Tree.TreeNode root){
        if(root==null){
            return root;
        }
